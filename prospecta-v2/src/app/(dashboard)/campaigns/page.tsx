...
// Modal rendered via portal to bypass any parent overflow:hidden
...
export default function CampaignsPage() {
    ...
    <div style={{ background: 'rgba(59,130,246,.08)', border: '1px solid rgba(59,130,246,.2)', borderRadius: 12, padding: '14px 16px', marginBottom: 16 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: 10, alignItems: 'flex-end' }}>
        ...
        <button onClick={() => generateAI(isEdit, idx)} disabled={aiLoading}
          style={{ padding: '8px 16px', background: aiLoading ? 'rgba(59,130,246,.4)' : 'linear-gradient(135deg,#3b82f6,#1d4ed8)', border: 'none', borderRadius: 9, color: '#fff', fontSize: 12, fontWeight: 700, cursor: aiLoading ? 'not-allowed' : 'pointer', whiteSpace: 'nowrap' }}>
  ...
  const StepCard = ({ step, idx, isEdit }) => {
    ...
    const upd = (field, val) => {
      if (isEdit) {
        ...
        const ns = [...steps];
        const k = field === 'delay_days' ? 'delayDays' : field;
        ns[idx] = { ...ns[idx], [k]: val };
        setSteps(ns);
      }
    };
    ...
    const remove = () => {
      if (isEdit) setEditData({ ...editData, steps: editData.steps.filter((_, i) => i !== idx) });
      else setSteps(p => p.filter((_, i) => i !== idx));
    };
    ...
    return (
      <div style={{ background: 'rgba(255,255,255,.04)', border: '0.5px solid rgba(255,255,255,.08)', borderRadius: 12, padding: '14px 16px', marginBottom: 10 }}>
            ...
            {idx > 0 && (
              <span style={{ fontSize: 12, color: 'rgba(255,255,255,.35)', display: 'flex', alignItems: 'center', gap: 5 }}>
                · J+<input type="number" min="1" value={delay}
                  onChange={e => upd(isEdit ? 'delay_days' : 'delayDays', +e.target.value)}
        ...
        <div style={{ marginBottom: 8 }}>
          ...
          <input value={subject} onChange={e => upd('subject', e.target.value)} style={inp} placeholder="Ex: Une idée pour accélérer votre croissance" />
        </div>
        <div>
          <label style={lbl}>Corps du message</label>
          <textarea value={body} onChange={e => upd('body', e.target.value)} rows={6}
  ...
  const ModalHeader = ({ title, subtitle, onClose, steps: wSteps }) => (
    <div style={{ padding: '18px 24px', borderBottom: '0.5px solid rgba(255,255,255,.08)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
      ...
      <button onClick={onClose}
        style={{ width: 32, height: 32, background: 'rgba(255,255,255,.06)', border: '0.5px solid rgba(255,255,255,.1)', borderRadius: 8, color: 'rgba(255,255,255,.5)', cursor: 'pointer', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
    </div>
  );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          ...
          <p style={{ fontSize: 12, color: 'rgba(255,255,255,.4)', marginTop: 4 }}>
            {campaigns.length} campagne(s) · Séquences de prospection automatisées
          </p>
        </div>
              ...
              <div key={c.id} style={{ ...glass, padding: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  ...
                  <div style={{ display: 'flex', gap: 5 }}>
                    <button onClick={() => handleEdit(c.id)} title="Modifier" style={{ padding: '5px 8px', background: 'rgba(96,165,250,.12)', border: '0.5px solid rgba(96,165,250,.25)', borderRadius: 7, color: '#60a5fa', fontSize: 13, cursor: 'pointer' }}>✎</button>
                    <button onClick={() => handleToggle(c.id, c.status)} title={c.status === 'RUNNING' ? 'Pause' : 'Lancer'}
                    ...
                    <button onClick={() => handleDelete(c.id)} title="Supprimer" style={{ padding: '5px 8px', background: 'rgba(248,113,113,.1)', border: '0.5px solid rgba(248,113,113,.2)', borderRadius: 7, color: '#f87171', fontSize: 13, cursor: 'pointer' }}>⌫</button>
                  </div>
                </div>
                <div>
      ...
      {editingId && editData && (
        <Modal onClose={() => { setEditingId(null); setEditData(null); }}>
          <ModalHeader title="Modifier la campagne" subtitle={editData.name} onClose={() => { setEditingId(null); setEditData(null); }} />
      ...
      {/* ─── Create wizard via portal ─── */}
      {showNew && (
        <Modal onClose={() => { setShowNew(false); setWizardStep(1); }}>
          <ModalHeader title="Nouvelle campagne" onClose={() => { setShowNew(false); setWizardStep(1); }} steps={true} />
          <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px' }}>
            {wizardStep === 1 && (
            ...
            {wizardStep === 2 && (
              <div>
                ...
                <div style={{ fontSize: 12, fontWeight: 700, color: '#f0f9ff', marginBottom: 10 }}>
                  ◈ Séquence d'emails ({steps.length} étape{steps.length > 1 ? 's' : ''})
                </div>
