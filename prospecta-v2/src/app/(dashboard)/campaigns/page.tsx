...
import { Mail, Plus, Play, Pause, Trash2, Sparkles, X, ChevronRight } from 'lucide-react';
...
type Step = { order: number; subject: string; body: string; delayDays: number };
...
export default function CampaignsPage() {
  ...
  const generateAI = async (idx: number) => {
    ...
    const res = await fetch('/api/ai', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'generate', company: aiConfig.company, jobTitle: aiConfig.jobTitle, goal: aiConfig.goal, tone: aiConfig.tone, language: aiConfig.language, prospectName: 'Prénom Nom' }),
    });
    const data = await res.json() as { data: { subject: string; body: string; provider: string } };
    if (data.data) {
      const updated = [...steps];
      updated[idx] = { ...updated[idx], subject: data.data.subject, body: data.data.body };
      setSteps(updated);
    ...
    <div className="space-y-5">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 py-20 text-center">
          ...
          <button onClick={() => setShowNew(true)} className="px-4 py-2 bg-blue-600 text-white rounded-xl text-sm font-semibold hover:bg-blue-700 transition">
            Créer une campagne
          </button>
        </div>
      ) : (
              ...
              <div key={c.id} className="bg-white rounded-2xl border border-slate-200/80 p-5 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => handleToggle(c.id, c.status)} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition" title={c.status === 'RUNNING' ? 'Pause' : 'Lancer'}>
              ...
              {wizardStep === 1 && (
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    {[
                      { key: 'name', label: 'Nom de la campagne *', placeholder: 'Prospection Q3 2025' },
                ...
                <div className="space-y-5">
                  ...
                  <div className="bg-blue-50 rounded-xl border border-blue-200 p-4">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-sm font-semibold text-blue-900">Générateur IA (Groq / Gemini)</span>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      {[
                        ...
                        <div key={f.key} className={f.key === 'goal' ? 'col-span-2' : ''}>
                          ...
                          <input value={(aiConfig as Record<string, string>)[f.key]} onChange={e => setAiConfig(p => ({ ...p, [f.key]: e.target.value }))}
                  ...
                  {steps.map((s, i) => (
                    <div key={i} className="border border-slate-200 rounded-xl p-4">
                          ...
                          {i > 0 && (
                            <div className="flex items-center gap-1 text-xs text-slate-500">
                              ...
                              <input type="number" min="0" value={s.delayDays}
                                ...
                                className="w-12 px-1.5 py-0.5 border border-slate-200 rounded text-center text-xs focus:outline-none" />
                              <span>jours</span>
                            </div>
                          )}
                        </div>
                        <div className="flex gap-1">
                          <button onClick={() => generateAI(i)} disabled={aiLoading}
                      ...
                      <textarea value={s.body} onChange={e => { const u = [...steps]; u[i] = { ...s, body: e.target.value }; setSteps(u); }}
                        ...
                        placeholder="Corps de l'email... Utilisez {{firstName}}, {{company}}" />
                    </div>
                  ))}
